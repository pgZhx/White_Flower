...
> 本文档根据项目 Prompt 整理，用于实现服务器权威的 Game Engine。
...
> ⚠️ 6 人局阈值存在待核验冲突：Prompt 后文又提到 “6-player sacrifice/death threshold = 4 / 5”。
> 当前实现按上方主表使用 4 / 4，并已配置化；最终发布前需用户确认。
...
| 10 | 所有人完成 PLAYER_ACTIONS 后、Reveal 前，发动者可选：不发动，或指定一名已经出牌且当前仍至少剩一张手牌的其他玩家；被选玩家收回本轮刚出的牌，从剩余手牌选择另一张不同的牌，暗置提交。 |
